...
<div class="max-w-2xl space-y-6">
    <div>
        ...
        <h1 class="text-2xl font-semibold text-brex-ink tracking-brex-24">Edit Jadwal Keberangkatan</h1>
        <p class="text-sm text-brex-pewter mt-1">Perbarui detail keberangkatan atau ubah status operasional.</p>
    </div>
        ...
        <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="route_id" id="route_id" required class="w-full bg-brex-paper border border-brex-mist rounded-brex px-3.5 py-2.5 text-brex-ink text-sm focus:outline-none focus:border-brex-ember focus:ring-1 focus:ring-brex-ember">
                    @foreach($routes as $r)
                        <option value="{{ $r->id }}" {{ old('route_id', $schedule->route_id) == $r->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="waktu_berangkat" id="waktu_berangkat" value="{{ old('waktu_berangkat', \Carbon\Carbon::parse($schedule->waktu_berangkat)->format('Y-m-d\TH:i')) }}" required
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full bg-brex-paper border border-brex-mist rounded-brex px-3.5 py-2.5 text-brex-ink text-sm focus:outline-none focus:border-brex-ember focus:ring-1 focus:ring-brex-ember">
                        <option value="scheduled" {{ old('status', $schedule->status) == 'scheduled' ? 'selected' : '' }}>Scheduled (Terjadwal)</option>
                        <option value="ongoing" {{ old('status', $schedule->status) == 'ongoing' ? 'selected' : '' }}>Ongoing (Dalam Perjalanan)</option>
